...
use bm13xx_asic::{register::ChipIdentification, Asic};
use bm13xx_protocol::{
...
impl<A: Asic, P: Read + Write, D: DelayNs> Chain<A, P, D> {
    pub fn new(asic_cnt: usize, asic: A, domain_cnt: usize, port: P, delay: D) -> Self {
        Chain::<A, P, D> {
            asic_cnt,
            asic,
    ...
    pub async fn enumerate(&mut self) -> Result<(), P::Error> {
        ...
        // }
        if asic_cnt > 0 {
            self.asic_addr_interval = 256 / asic_cnt;
        }
        ...
        self.delay.delay_ms(10).await;
        for i in 0..asic_cnt {
            let cmd = Command::set_chip_addr((i * self.asic_addr_interval) as u8);
            self.port.write_all(&cmd).await.map_err(Error::Io)?;
            self.delay.delay_ms(2).await;
        }
        Ok(())
